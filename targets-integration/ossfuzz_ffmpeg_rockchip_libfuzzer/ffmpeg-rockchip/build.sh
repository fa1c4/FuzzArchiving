...
make -j"$(nproc)" RM=: install || echo "WARNING: make install (zlib) failed (ignored)"

cd $SRC/libxml2
./autogen.sh --prefix="$FFMPEG_DEPS_PATH" --enable-static \
      --without-debug --without-ftp --without-http \
      --without-legacy --without-python
make -j"$(nproc)" RM=: install || echo "WARNING: make install (libxml2) failed (ignored)"
...
make -j"$(nproc)" RM=: install || echo "WARNING: make install (libass) failed (ignored)"
...
cd alsa-lib-*
./configure --prefix="$FFMPEG_DEPS_PATH" --enable-static --disable-shared
make -j"$(nproc)" RM=: install || echo "WARNING: make install (alsa-lib) failed (ignored)"

cd $SRC/fdk-aac
autoreconf -fiv
CXXFLAGS="$CXXFLAGS -fno-sanitize=shift-base,signed-integer-overflow" \
./configure --prefix="$FFMPEG_DEPS_PATH" --disable-shared
make -j"$(nproc)" RM=: install || echo "WARNING: make install (fdk-aac) failed (ignored)"
...
make -j"$(nproc)" RM=: install || echo "WARNING: make install (libvpx) failed (ignored)"

cd $SRC/ogg
./autogen.sh
./configure --prefix="$FFMPEG_DEPS_PATH" --enable-static --disable-crc
make -j"$(nproc)" RM=: install || echo "WARNING: make install (ogg) failed (ignored)"

cd $SRC/opus
./autogen.sh
./configure --prefix="$FFMPEG_DEPS_PATH" --enable-static
make -j"$(nproc)" RM=: install || echo "WARNING: make install (opus) failed (ignored)"
...
make -j"$(nproc)" RM=: install || echo "WARNING: make install (theora) failed (ignored)"

cd $SRC/vorbis
./autogen.sh
./configure --prefix="$FFMPEG_DEPS_PATH" --enable-static
make -j"$(nproc)" RM=: install || echo "WARNING: make install (vorbis) failed (ignored)"

# Remove shared libraries to avoid accidental linking against them.
rm $FFMPEG_DEPS_PATH/lib/*.so || true
rm $FFMPEG_DEPS_PATH/lib/*.so.* || true
...
make RM=: fate-rsync SAMPLES=$TEST_SAMPLES_PATH || echo "WARNING: make fate-rsync failed (ignored)"
...
make -j"$(nproc)" RM=: "${ALL_TARGETS[@]}" || echo "WARNING: make fuzzers (ALL_TARGETS) failed (ignored)"

for i in "${!BSF_TARGETS[@]}"; do
      echo -en "[libfuzzer]\nmax_len = 1000000\n" >"$OUT/${BSF_FUZZER_NAMES[$i]}.options"
      mv "${BSF_TARGETS[$i]}" "$OUT/${BSF_FUZZER_NAMES[$i]}" || echo "WARNING: mv BSF target failed (ignored)"
done

for i in "${!DECODER_TARGETS[@]}"; do
      echo -en "[libfuzzer]\nmax_len = 1000000\n" >"$OUT/${DECODER_FUZZER_NAMES[$i]}.options"
      mv "${DECODER_TARGETS[$i]}" "$OUT/${DECODER_FUZZER_NAMES[$i]}" || echo "WARNING: mv DECODER target failed (ignored)"
done

for i in "${!ENCODER_TARGETS[@]}"; do
      echo -en "[libfuzzer]\nmax_len = 1000000\n" >"$OUT/${ENCODER_FUZZER_NAMES[$i]}.options"
      mv "${ENCODER_TARGETS[$i]}" "$OUT/${ENCODER_FUZZER_NAMES[$i]}" || echo "WARNING: mv ENCODER target failed (ignored)"
done

# Move fuzzer for sws
fuzzer_name=$($SRC/name_mappings.py binary_name other SWS)
echo -en "[libfuzzer]\nmax_len = 1000000\n" >"$OUT/${fuzzer_name}.options"
mv tools/target_sws_fuzzer "$OUT/${fuzzer_name}" || echo "WARNING: mv SWS fuzzer failed (ignored)"

# Move fuzzer for swr
fuzzer_name=$($SRC/name_mappings.py binary_name other SWR)
echo -en "[libfuzzer]\nmax_len = 1000000\n" >"$OUT/${fuzzer_name}.options"
mv tools/target_swr_fuzzer "$OUT/${fuzzer_name}" || echo "WARNING: mv SWR fuzzer failed (ignored)"

# Move fuzzer for demuxer
fuzzer_name=$($SRC/name_mappings.py binary_name other DEM)
echo -en "[libfuzzer]\nmax_len = 1000000\n" >"$OUT/${fuzzer_name}.options"
mv tools/target_dem_fuzzer "$OUT/${fuzzer_name}" || echo "WARNING: mv DEM fuzzer failed (ignored)"

# We do not need raw reference files for the muxer
rm $(find fate-suite -name '*.s16') || true
rm $(find fate-suite -name '*.dec') || true
rm $(find fate-suite -name '*.pcm') || true
...
mv tools/target_io_dem_fuzzer "$OUT/${fuzzer_name}" || echo "WARNING: mv IO_DEM fuzzer failed (ignored)"

# Clean before reconfiguring for demuxers
make RM=: distclean || echo "WARNING: make distclean failed (ignored for oss-fuzz)"
...
make -j"$(nproc)" RM=: "${DEMUXER_TARGETS[@]}" || echo "WARNING: make fuzzers (DEMUXER_TARGETS) failed (ignored)"

for i in "${!DEMUXER_TARGETS[@]}"; do
      mv "${DEMUXER_TARGETS[$i]}" "$OUT/${DEMUXER_FUZZER_NAMES[$i]}" || echo "WARNING: mv DEMUXER fuzzer failed (ignored)"
done
